  ...
  try {
    switch (body.action) {
      ...
      case 'saveUserPermissions': {
        if (typeof body.userId !== 'string' || !isUserPermissionsInput(body.permissions)) {
          return jsonResponse({ error: 'Invalid request.' }, 400);
        }

        await saveUserPermissions(db, guard.auth, body.userId, body.permissions);
...
async function saveUserPermissions(
  ...
  const targetUser = await db
    .prepare(
      'SELECT u.display_name, m.role FROM app_users u JOIN workspace_memberships m ON m.user_id = u.id WHERE m.workspace_id = ? AND u.id = ?',
    )
    .bind(actor.workspace.id, userId)
    .first<{ display_name: string; role: AuthRole }>();

  if (targetUser === null || targetUser.role === 'owner' || targetUser.role === 'admin') {
    throw new Error('forbidden');
  }

  await saveSharedView(db, actor, {
    id: getGeneratedUserAccessSharedViewId(actor.workspace.id, userId),
    name: `User access - ${targetUser.display_name}`,
    description: 'Generated by simple per-user permissions.',
    active: true,
    allMembers: permissions.allMembers,
    allCategories: permissions.allCategories,
    userIds: [userId],
    memberIds: permissions.allMembers ? [] : permissions.memberIds,
    categories: permissions.allCategories ? [] : permissions.categories,
    permissions: normalizePermissionDependencies(permissions.permissions),
  });
}
...
function isUserPermissionsInput(value: unknown): value is UserPermissionsInput {
  if (typeof value !== 'object' || value === null) {
    return false;
  }

  const permissions = value as Partial<UserPermissionsInput>;

  return (
    typeof permissions.allMembers === 'boolean' &&
    typeof permissions.allCategories === 'boolean' &&
    Array.isArray(permissions.memberIds) &&
    permissions.memberIds.every((id) => typeof id === 'string' && id.trim() !== '') &&
    Array.isArray(permissions.categories) &&
    permissions.categories.every(isEventCategory) &&
    Array.isArray(permissions.permissions) &&
    permissions.permissions.every(isSharedViewPermission)
  );
}
...
function normalizePermissionDependencies(permissions: PermissionScope[]): PermissionScope[] {
  ...
  return sharedViewPermissionOrder.filter((permission) => selected.has(permission));
}

function getGeneratedUserAccessSharedViewId(workspaceId: string, userId: string): string {
  return `user-access-${workspaceId}-${userId}`;
}

const sharedViewPermissionOrder: PermissionScope[] = [
  'view_schedule',
  'edit_schedule',
  'view_transportation',
  'edit_transportation',
